...
import quadcopter_vel_control.tasks.direct.quadcopter_vel_control  # noqa: F401
...
def save_reproducibility_info(log_dir, agent, args_cli, env_cfg):
    ...
    files_to_copy = [
        ...
        # Env
        os.path.abspath(os.path.join(os.path.dirname(__file__), "../../source/quadcopter_vel_control/quadcopter_vel_control/tasks/direct/quadcopter_vel_control/quadcopter_vel_control_env.py")),
        os.path.abspath(os.path.join(os.path.dirname(__file__), "../../source/quadcopter_vel_control/quadcopter_vel_control/tasks/direct/quadcopter_vel_control/quadcopter_vel_control_env_cfg.py")),
    ]
...
def main(env_cfg, agent_cfg: dict):
    ...
    log_dir = os.path.join("projects", "quadcopter_vel_control", "logs", "skrl", "quadcopter_vel_control_direct", experiment_name)
    ...
    agent_config = agent.cfg
    default_timesteps = 50000 # ~25M steps
    ...
    try:
        ...
        print("[INFO] Training interrupted by user. Saving stats before exiting...")
    finally:
        # This block runs even if training finishes normally OR crashes/interrupts
        if hasattr(agent, "_state_preprocessor"):
            scaler_stats = {
                "running_mean": agent._state_preprocessor.running_mean.detach().cpu(),
                "running_variance": agent._state_preprocessor.running_variance.detach().cpu(),
                "epsilon": float(agent._state_preprocessor.epsilon),
                "clip_threshold": float(agent._state_preprocessor.clip_threshold),
            }
            scaler_path = os.path.join(target_repro_log_dir, "scaler_stats.pt")
            torch.save(scaler_stats, scaler_path)
            print(f"[INFO] Saved scaler stats to {scaler_path}")
        else:
            print("[WARNING] Agent has no state_preprocessor. No stats saved.")
