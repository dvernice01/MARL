...
parser.add_argument("--task", type=str, default="Template-Velocity-Controller-Direct-v0", help="Name of the task.")
...
import QuadcopterVae.tasks.direct.quadcoptervae  # noqa: F401
...
def save_reproducibility_info(log_dir, agent, args_cli, env_cfg):
    ...
    files_to_copy = [
        ...
        # Env
        os.path.abspath(os.path.join(os.path.dirname(__file__), "../../source/QuadcopterVae/QuadcopterVae/tasks/direct/quadcoptervae/quadcoptervae_env.py")),
        os.path.abspath(os.path.join(os.path.dirname(__file__), "../../source/QuadcopterVae/QuadcopterVae/tasks/direct/quadcoptervae/quadcoptervae_env_cfg.py")),
    ]
...
#     """Train with manual agent instantiation."""

#     run = wandb.init(project="quadcopter_hierarchical_control")
#     sweep_cfg = wandb.config  # ← this contains the sweep-chosen values
    
#     # Apply reward scales to env_cfg
#     if hasattr(sweep_cfg, "lin_vel_reward_scale"):
#         env_cfg.lin_vel_reward_scale = sweep_cfg.lin_vel_reward_scale
#     if hasattr(sweep_cfg, "ang_vel_reward_scale"):
#         env_cfg.ang_vel_reward_scale = sweep_cfg.ang_vel_reward_scale
#     if hasattr(sweep_cfg, "distance_to_goal_reward_scale"):
#         env_cfg.distance_to_goal_reward_scale = sweep_cfg.distance_to_goal_reward_scale
#     if hasattr(sweep_cfg, "alive_reward_scale"):
#         env_cfg.alive_reward_scale = sweep_cfg.alive_reward_scale
#     if hasattr(sweep_cfg, "death_reward_scale"):
...
#     # Settings
#     experiment_name = args_cli.experiment_name if args_cli.experiment_name else datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + f"_Manual_{args_cli.agent.upper()}"
#     log_dir = os.path.join("projects", "quadcopter_hierarchical_control", "logs", "skrl", "quadcopter_hierarchical_control_direct", experiment_name)
...
#     # 2. Instantiate Agent (Modular)
#     # Get device from environment
#     device = env.device
    
#     print(f"[INFO] Instantiating {args_cli.agent.upper()} Agent (Modular)...")
...
#     # We save it in the agent's experiment directory (timestamped)
#     target_repro_log_dir = agent.experiment_dir if hasattr(agent, "experiment_dir") else log_dir
#     save_reproducibility_info(target_repro_log_dir, agent, args_cli, env_cfg)
...
#     # 3. Trainer
#     print("[INFO] Starting Trainer...")
    
#     agent_config = agent.cfg
#     default_timesteps = 80000 # ~25M steps
    
#     # Construct Trainer Config manually
#     trainer_cfg = {
...
#         trainer_cfg["timesteps"] = args_cli.max_iterations * agent_config["rollouts"]
...
def main(env_cfg, agent_cfg: dict):
    ...
    if "death_reward_scale" in sweep_cfg:
        env_cfg.death_reward_scale = -sweep_cfg["death_reward_scale"]
    ...
    log_dir = os.path.join("projects", "QuadcopterVae", "logs", "skrl",
                           "QuadcopterVae_direct", experiment_name)
