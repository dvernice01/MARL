...
def plot_3d_voxels(data: np.ndarray, title_prefix: str, save_path: str | None = None):
    ...
    ax1.set_xlim(0, data.shape[3] - 1)
    ax1.set_ylim(0, data.shape[2] - 1)
    ax1.set_zlim(0, data.shape[1] - 1)
    ax1.scatter(ix, iy, iz, s=20, c="red", alpha=0.4, marker="s")
    ...
    vals = svs[iz, iy, ix]
    ax2.set_xlim(0, data.shape[3] - 1)
    ax2.set_ylim(0, data.shape[2] - 1)
    ax2.set_zlim(0, data.shape[1] - 1)
    sc = ax2.scatter(ix, iy, iz, s=20, c=vals, cmap="viridis", alpha=0.5, marker="s")
